...
void sha256_transform(uint32_t *state, const uint8_t data[64])
{
    uint32_t m[64], w[64];
...
void sha256_final(uint32_t *state, uint8_t *digest)
{
    ...
    static uint32_t temp_len = 0;
    uint32_t i, j;
    uint64_t bit_len = temp_len * 8;

    temp[temp_len++] = 0x80;
    if (temp_len > 56) {
        while (temp_len < 64) temp[temp_len++] = 0;
        sha256_transform(state, temp);
        temp_len = 0;
    }
    while (temp_len < 56) temp[temp_len++] = 0;

    for (i = 0; i < 8; i++) {
        temp[56 + i] = (bit_len >> (56 - i * 8)) & 0xff;
    }
